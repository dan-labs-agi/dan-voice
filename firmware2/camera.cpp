...
CameraStatus init() {
    ...
    if (psramFound()) {
        ...
        Serial.println("[Camera] WARNING: PSRAM not found, falling back to reduced settings");
    ...
    if (sensor == nullptr) {
        Serial.println("[Camera] Failed to get sensor handle after init");
    ...
    Serial.printf("[Camera] Frame size=%d, quality=%d, fb_count=%d\n",
...
camera_fb_t* capture() {
    ...
    if (fb == nullptr) {
        Serial.println("[Camera] esp_camera_fb_get() returned null (capture failed)");
    }
    return fb;
}
