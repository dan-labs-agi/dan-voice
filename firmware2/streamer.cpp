...
void cameraTask(void* pvParameters) {
    Serial.println("[CameraTask] Started on core " + String(xPortGetCoreID()));
...
void networkTask(void* pvParameters) {
    ...
    if (!s_transport->begin()) {
        Serial.println("[NetworkTask] FATAL: transport begin() failed, retrying in loop");
    }
...
void monitorTask(void* pvParameters) {
    ...
    for (;;) {
        ...
        Serial.printf(
            "[Stats] FPS(cur=%.1f avg=%.1f) cap=%uus net=%uus queue=%u dropped=%u "
...
void begin() {
    ...
    if (s_frameQueue == nullptr) {
        Serial.println("[Streamer] FATAL: failed to create frame queue");
        return;
    }
    ...
    Serial.println("[Streamer] All tasks started");
}
