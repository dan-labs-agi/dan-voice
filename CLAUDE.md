...
## Out-of-scope addition: camera/WiFi streaming (`components/camera_streamer/`)

`firmware/` also carries a second, unrelated firmware capability: a native
ESP-IDF ESP32-S3 camera streamer (originally developed standalone as an
Arduino sketch in `firmware2/` — TCP JPEG streaming over Wi-Fi STA — then
ported to plain ESP-IDF/esp_wifi/esp32-camera, no Arduino framework
involved) integrated onto the **same physical board** as the BLE
provisioning firmware, running **simultaneously** with it (both BLE and
Wi-Fi active at once, real radio coexistence — not time-sliced sequential
operation). Lives in `firmware/components/camera_streamer/`, wired into
`main.c`'s `app_main()` via a single `camera_streamer_start()` call
alongside the NimBLE init — no Arduino HAL, no Arduino-as-ESP-IDF-component
dependency.

An Arduino-as-ESP-IDF-component approach was evaluated and approved first,
then discarded mid-implementation before anything was built/flashed (see
PROGRESS.md's "camera/WiFi streaming integration" entry for the full
arc) in favor of this native port, once assessment showed most of
`firmware2/`'s code was already framework-agnostic and the native port was
meaningfully lower-risk (no two-framework init interleaving, smaller
partition footprint) for a modest amount of extra rewrite work.

This is a **deliberate, permanent architectural coupling**, not part of the
original 9-phase BLE provisioning plan above and not a step toward voice/AI
functionality. It exists because the hardware only has one board available.
Call this out explicitly if it's ever unclear why camera/streaming code
lives inside what's otherwise a pure auth/pairing firmware project — it was
a conscious scope addition, not scope creep to be "cleaned up" later.
