...
// A raw socket (rather than a higher-level wrapper) is used so we have
// direct control over SO_SNDBUF, TCP_NODELAY, and send timeouts — all of
// which matter for sustained high-frame-rate throughput.
// ----------------------------------------------------------------------------
...
public:
    ...
    bool waitForClient(uint32_t timeoutMs) override {
        ...
        ESP_LOGI(TAG, "Client connected: %s:%d", ipStr, ntohs(clientAddr.sin_port));
        ...
        while (sent < len) {
            ...
            if (n < 0) {
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    ...
                    ESP_LOGW(TAG, "send() timed out, dropping client");
                } else {
...
bool sendFrame(ITransport& transport, const uint8_t* jpegData, size_t jpegLen) {
    if (jpegLen == 0 || jpegLen > PROTOCOL_MAX_FRAME_BYTES) {
        ESP_LOGW(TAG, "Refusing to send invalid frame size=%u", (unsigned)jpegLen);
        return false;
    }
